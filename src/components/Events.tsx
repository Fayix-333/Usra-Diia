      ...
      try {
        ...
        if (saved) {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
            ...
            for (const item of initialEventsData) {
              if (!seen.has(item.id)) {
                seen.add(item.id);
                combined.push(item);
              }
  ...
  return (
    <section id="events" className="relative py-12 md:py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto overflow-hidden">
      ...
      >
        <AnimatePresence>
          {filteredEvents
            .filter((event, idx, self) => idx === self.findIndex((e) => e.id === event.id))
            .map((event, index) => {
              const isUpcoming = event.status === 'upcoming';

              return (
                <motion.div
                  key={event.id ? `event-card-${event.id}` : `event-idx-${index}`}
                layout
              ...
              <div className="w-full md:w-1/2 p-6 sm:p-8 flex flex-col justify-between border-t md:border-t-0 md:border-l border-white/5">
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    ...
                    {selectedEvent.tags?.map((t, tIdx) => (
                      <span key={`${selectedEvent.id}-tag-${tIdx}-${t}`} className="px-2.5 py-0.5 rounded-full text-[11px] font-mono text-neutral-400 bg-white/5 border border-white/5">
                        #{t}
                      </span>
                    ))}
                  </div>
                    ...
                    <div className="mb-6">
                      ...
                      <ul className="space-y-1.5">
                        {selectedEvent.highlights.map((h, i) => (
                          <li key={`${selectedEvent.id}-hl-${i}`} className="text-xs text-neutral-300 flex items-start gap-2">
