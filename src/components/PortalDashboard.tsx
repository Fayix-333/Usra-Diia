...
export default function PortalDashboard({ isOpen, onClose }: PortalDashboardProps) {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    e.preventDefault();
    const targetId = currentUser?.adNo || currentUser?.username || currentUser?.id;
    if (!targetId) {
      setPasswordMsg({ type: 'error', text: 'No active user session detected.' });
      return;
    }
    ...
    setPasswordMsg(null);
    const res = await changePassword(newPassword, targetId);
    setIsChangingPass(false);

    if (res.success) {
      setPasswordMsg({ 
        type: 'success', 
        text: 'Password updated successfully! Next time you log in, please use your new password.' 
      });
      setNewPassword('');
      setConfirmPassword('');
      setTimeout(() => {
        ...
        >
          ...
          <div className="p-6 pb-4 border-b border-white/10 flex flex-wrap items-center justify-between gap-4 bg-neutral-950/70">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Change Password quick button for any authenticated user */}
              {currentUser && (
                <button
                  onClick={() => setShowChangePassword(prev => !prev)}
                  className={`px-3 py-2 rounded-xl text-xs font-mono flex items-center gap-1.5 transition-all border cursor-pointer ${
                    showChangePassword
                      ? 'bg-cyan-600 text-white border-cyan-400 shadow-lg shadow-cyan-500/25 font-bold'
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {/* UNIVERSAL PASSWORD CHANGING PANEL (STUDENTS, FACULTY & CAMPUS MEMBERS) */}
            <AnimatePresence>
              {showChangePassword && (
                <motion.form
                  initial={{ opacity: 0, height: 0 }}
                  ...
                  className="p-6 rounded-2xl bg-neutral-900/95 border border-cyan-500/40 space-y-4 shadow-2xl relative overflow-hidden"
                >
                  <div className="flex items-center justify-between border-b border-white/5 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <h4 className="font-display font-bold text-sm text-white">Change Account Password</h4>
                        <p className="text-[11px] text-neutral-400">
                          Updated securely in Firebase Firestore. Your next login will use this new password.
                        </p>
                      </div>
                    </div>
                    <span className="px-2.5 py-1 rounded-lg text-[10px] font-mono bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
                      User: {currentUser.name} {currentUser.adNo ? `(Ad: ${currentUser.adNo})` : `(${currentUser.username})`}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <span className="text-[11px] text-neutral-500 font-mono">
                      Tip: Remember this password. It will be required immediately on your next sign in.
                    </span>
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
