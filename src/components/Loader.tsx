...
export default function Loader({ onComplete }: LoaderProps) {
  ...
  useEffect(() => {
    // If user already visited during this browser session, load almost immediately
    const hasVisited = Boolean(sessionStorage.getItem('usra_visited_session'));
    sessionStorage.setItem('usra_visited_session', 'true');

    const stepSpeed = hasVisited ? 15 : 25;
