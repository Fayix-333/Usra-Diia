...
interface AuthContextType {
  ...
  // Student & Universal Password Modification (Recorded to Firestore for Admin)
  changeStudentPassword: (adNo: string, newPassword: string) => Promise<{ success: boolean; error?: string }>;
  changePassword: (newPassword: string, targetIdentifier?: string) => Promise<{ success: boolean; error?: string }>;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    async function loadAnnouncements() {
      try {
        ...
        if (!snap.empty) {
          ...
          const uniqueAnnouncements = list.filter(a => {
            if (!a.id || seenAnnIds.has(a.id)) return false;
            seenAnnIds.add(a.id);
            return true;
          });
  ...
  const saveUserSession = (user: AuthUser) => {
    ...
    // Mirror to Firestore users collection in the background without blocking login speed
    setDoc(doc(db, 'users', user.id), {
      ...
      console.warn('[Firestore] User record background sync note:', e);
    });
  };

  // 1. High-Speed Login for the 27 Students
  const loginStudent27 = async (adNo: string, pass: string): Promise<{ success: boolean; error?: string }> => {
    ...
    // Fast-path: Check local credentials cache
    let cachedPassword = '';
    try {
      ...
      if (localCache) {
        ...
        if (parsed[matchedStudent.adNo]) {
          cachedPassword = parsed[matchedStudent.adNo];
        }
      }
    } catch (e) {}

    // If custom password exists in cache and matches, instant entry!
    if (cachedPassword && cleanPass === cachedPassword) {
      saveUserSession(studentUser);
      setIsLoading(false);
      return { success: true };
    }

    // Default password check: matches admission number, roll number, or 'ad'+adNo
    const validDefaultMatches = [
    ...
    if (!cachedPassword && validDefaultMatches.includes(cleanPass.toLowerCase())) {
      saveUserSession(studentUser);
      setIsLoading(false);
      return { success: true };
    }

    // If local cache and default did not match, check Firestore with a 2-second timeout
    try {
      const timeoutPromise = new Promise<null>((_, reject) => 
        setTimeout(() => reject(new Error('timeout')), 2200)
      );
      const fetchPromise = getDoc(doc(db, 'studentCredentials', matchedStudent.adNo));
      const credSnap = await Promise.race([fetchPromise, timeoutPromise]) as any;
      ...
      console.log('[Auth] Firestore credentials check note:', err);
    }
  ...
  const changeStudentPassword = async (adNo: string, newPassword: string): Promise<{ success: boolean; error?: string }> => {
    ...
    // 1. Immediately cache password locally so sign in is instantaneous
    try {
      const localCache = localStorage.getItem('usra_student_passwords');
      const existing = localCache ? JSON.parse(localCache) : {};
    ...
    // 2. Persist to Firebase Firestore studentCredentials collection
    try {
      ...
      console.warn('[Firestore] Storing credentials note:', err);
    }

    // 3. Update session if this student is currently logged in
    if (currentUser && currentUser.adNo === cleanAdNo) {
      const updatedUser = { ...currentUser };
      setCurrentUser(updatedUser);
      localStorage.setItem('usra_active_user', JSON.stringify(updatedUser));
    }
  ...
  const sendClassMessage = async (messageText: string, autoExpire1Day: boolean = true): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      setClassMessages(prev => [savedMsg, ...prev.filter(m => m.id !== docRef.id)]);
      return { success: true };
    } catch (err) {
      console.warn('[Firestore] Message sent locally:', err);
      setClassMessages(prev => [newMsg, ...prev.filter(m => m.id !== newMsg.id)]);
      return { success: true };
    }
  };
  ...
  // 2. High-Speed Login for Usthad (fsl / fsl or custom password)
  const loginUsthadFsl = async (username: string, pass: string): Promise<{ success: boolean; error?: string }> => {
    ...
    if (cleanUser !== 'fsl' && cleanUser !== 'usthad') {
      setIsLoading(false);
      return { success: false, error: 'Invalid username. Class Teacher username is "fsl".' };
    }
    ...
    // Check custom password from local storage or default 'fsl'
    let usthadPass = 'fsl';
    try {
      const custom = localStorage.getItem('usra_usthad_pass');
      if (custom) usthadPass = custom;
    } catch {}

    if (cleanPass === usthadPass || cleanPass.toLowerCase() === 'fsl') {
      saveUserSession(usthadUser);
      setIsLoading(false);
      return { success: true };
    }

    // Check Firestore userCredentials/usthad-fsl with timeout
    try {
      const timeoutPromise = new Promise<null>((_, reject) => setTimeout(() => reject(new Error('timeout')), 2000));
      const fetchPromise = getDoc(doc(db, 'userCredentials', 'usthad-fsl'));
      ...
      if (snap && snap.exists()) {
        ...
        if (data?.password && data.password === cleanPass) {
          try {
            localStorage.setItem('usra_usthad_pass', cleanPass);
          } catch {}
          saveUserSession(usthadUser);
          setIsLoading(false);
          return { success: true };
  ...
  // 3. High-Speed Login for Other All Students & Ustads
  const loginGeneral = async ({ idOrEmail, pass, role }: { idOrEmail: string; pass: string; role: 'student_general' | 'usthad_general' }): Promise<{ success: boolean; error?: string }> => {
    ...
    // Check stored general users in localStorage
    ...
    // Check Firestore credentials if not matched in local cache
    try {
      const timeoutPromise = new Promise<null>((_, reject) => setTimeout(() => reject(new Error('timeout')), 2000));
      const fetchPromise = getDoc(doc(db, 'userCredentials', cleanId.toLowerCase()));
      const snap = await Promise.race([fetchPromise, timeoutPromise]) as any;
      if (snap && snap.exists() && snap.data()?.password === cleanPass) {
        const user: AuthUser = {
          id: snap.data().userId || `gen-${Date.now()}`,
          username: cleanId,
          name: snap.data().username || cleanId,
          role: role,
          createdAt: new Date().toISOString()
        };
        generalUsers[cleanId.toLowerCase()] = { ...user, pass: cleanPass };
    ...
    // Allow default instant demo entry with supplied details
  ...
  // Universal password change: handles students, usthad, general campus members, or explicit target
  const changePassword = async (newPassword: string, targetIdentifier?: string): Promise<{ success: boolean; error?: string }> => {
    const cleanPass = newPassword.trim();
    if (!cleanPass || cleanPass.length < 4) {
      return { success: false, error: 'Password must be at least 4 characters long.' };
    }

    const target = targetIdentifier ? targetIdentifier.trim() : (currentUser?.adNo || currentUser?.username || currentUser?.id || '');
    if (!target) {
      return { success: false, error: 'No user account specified for password change.' };
    }

    // Check if target is student (by admission number or role)
    const student = findStudentByAdNo(target);
    if (student || /^\d+$/.test(target) || (currentUser && currentUser.role === 'student_27')) {
      const adNo = student?.adNo || target;
      return changeStudentPassword(adNo, cleanPass);
    }

    // Check if target is Usthad
    if (target.toLowerCase() === 'fsl' || target.toLowerCase() === 'usthad' || currentUser?.role === 'usthad_fsl') {
      try {
        localStorage.setItem('usra_usthad_pass', cleanPass);
        await setDoc(doc(db, 'userCredentials', 'usthad-fsl'), {
          userId: 'usthad-fsl',
          username: 'fsl',
          password: cleanPass,
          updatedAt: new Date().toISOString()
    ...
    try {
      const generalUsersRaw = localStorage.getItem('usra_general_users');
      const generalUsers: Record<string, any> = generalUsersRaw ? JSON.parse(generalUsersRaw) : {};
      if (generalUsers[target.toLowerCase()]) {
        generalUsers[target.toLowerCase()].pass = cleanPass;
        localStorage.setItem('usra_general_users', JSON.stringify(generalUsers));
      }
      await setDoc(doc(db, 'userCredentials', target.toLowerCase()), {
        userId: target,
        username: target,
        password: cleanPass,
        updatedAt: new Date().toISOString()
